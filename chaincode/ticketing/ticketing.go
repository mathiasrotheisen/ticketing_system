...
// Ticketing specifies chaincode for a musical event ticketing system
...
func (t *Ticketing) Init(stub shim.ChaincodeStubInterface) sc.Response {
...
func (t *Ticketing) Invoke(stub shim.ChaincodeStubInterface) sc.Response {
...
func (t *Ticketing) generateTicket(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	var err error
	// id price day seat
	if len(args) != 4 {
		fmt.Println("Ticketing: generateTicket <id> <price> <day> <seat>")
		return shim.Error("Incorrent number of arguments. Expecting 4")
	}

	fmt.Println("Ticketing: Generating ticket initiated --")
	if len(args[0]) <= 0 {
...
func (t *Ticketing) transferTicket(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	//  "id" "owner"
	if len(args) != 2 {
	...
	fmt.Println("Ticketing: Transfer ticket initiated --")
	tvalbytes, err := stub.GetState(id)
	if err != nil {
...
func (t *Ticketing) queryTicket(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	var response string
	if len(args) != 1 {
...
func (t *Ticketing) queryTicketByOwner(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	// owner
	if len(args) < 1 {
...
func (t *Ticketing) repriceTicket(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	var response string
	//  "id" "new price"
	if len(args) != 2 {
	...
	fmt.Println("Ticketing: repricing ticket initiated -")
	if err != nil {
		response = ""
	}
...
func (t *Ticketing) queryAllTickets(stub shim.ChaincodeStubInterface) sc.Response {
	startKey := ""
	endKey := ""
...
func resultFromQuery(stub shim.ChaincodeStubInterface, query string) ([]byte, error) {
	var buffer bytes.Buffer
	fmt.Printf("Ticketing: resultFromQuery:\n%s-\n", query)
...
func (t *Ticketing) ticketHistory(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	if len(args) < 1 {
	...
	fmt.Printf("Ticketing: ticketHistory: %s\n", id)
...
func (t *Ticketing) lockTicket(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	var response string
	if len(args) < 1 {
		return shim.Error("Incorrect number of arguments. Expecting 1.")
	}
...
func (t *Ticketing) deleteTicket(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	var response string
	// id
...
func main() {
	if err := shim.Start(new(Ticketing)); err != nil {
		fmt.Printf("Error starting Ticketing chaincode: %s", err)
	}
}
